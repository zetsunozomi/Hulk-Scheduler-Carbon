...
DEFAULTS = {'episodes_per_budget':32,'epochs':4,'minibatch_episodes':32,'learning_rate':3e-4,
            ...
            'budgets':[1.,1.25,1.5,2.],'forecast_mode':'window','objective':'robust'}
...
def method_name(settings):
    name = 'ScaleDown' if settings['forecast_mode']=='window' else 'Current-CI'
...
def check_policy_inputs(metadata, encoder, predictor, bundle):
    ...
    require(metadata['predictor_sha256']==predictor.version, 'Checkpoint wait model differs')
...
def collect_episode(bundle, episode, model, encoder, generator, base, method, seed, chunk_stream, identity):
    ...
    summary = {**env.summary(),**identity,'policy_rule':'categorical_sampling'}
...
def train_policy(bundle, output, predictor_path, references_path, settings, resume=None):
    ...
    references,predictor = shared_inputs(bundle,predictor_path,references_path)
    encoder = PolicyInputs(bundle,predictor,references,settings['forecast_mode'])
    ...
    metadata = {'purpose':bundle.raw['purpose'],'settings':settings,'feature_schema':encoder.schema(),
                'predictor_sha256':predictor.version,'references':references,'software':source,
                ...
                for beta in settings['budgets']:
                    for index, original in enumerate(arrivals):
                        ...
                        identity = {**current,'epsilon':settings['epsilon'],'policy_checkpoint':f'training-pre-update-{iteration}',
                                    'power_objective':settings['objective'],'policy_forecast_mode':settings['forecast_mode']}
                        steps,returns,summary = collect_episode(bundle,episode,model,encoder,generator,cache.get(episode),
                                                                method_name(settings)+'-train',settings['seed'],chunks,identity)
...
def evaluate_policy(bundle, output, predictor_path, checkpoint_path, split='validation', budgets=None, sampling_seed=None):
    ...
    references,predictor = shared_inputs(bundle,predictor_path,metadata['references'])
    encoder = PolicyInputs(bundle,predictor,references,settings['forecast_mode'])
    ...
    manifest = {**bundle.manifest,'kind':'policy_evaluation','status':'running','split':split,
                ...
                'predictor_sha256':predictor.version,'software':software,
