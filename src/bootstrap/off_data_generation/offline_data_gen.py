...
def pre_simulate(simulator_start_time, workload_start_time, workload_end_time, slurm_config, backfill_config,
    ...
    num_nodes = slurm_config.get("num_nodes", 88)
    slurm_simulator.init_scheduler(num_nodes, slurm_config, simulator_start_time, backfill_config, queue_time_len)
...
def simulate(num_samples, start_time_interval, parallel, workload_name, sim_init_time, workload_len,
    ...
    if not parallel:
        ...
        short_temp_dir = "/tmp/sr"
        if cpu_cores == -1:
            ray.init(
                _temp_dir=short_temp_dir,
                runtime_env={"py_modules": [sim], "working_dir": "../",
                                  "excludes": ["/test/", "/data/", "/misc/", "/queue_prediction/json_data/","*.pt"] + file})
        else:
            ray.init(
                _temp_dir=short_temp_dir,
                runtime_env={"py_modules": [sim], "working_dir": "../",
