...
class Simulator:
    def __init__(self, mode=Mode.MINUTES):
        ...
        self._step_priority = 0

    def load_jobs(self, log):
        self._jobs = simparser.SimParser.parse_job(log)

    def load_pickle_jobs(self, base_log, log):
        self._jobs = simparser.SimParser.parse_job(base_log)
        regr = self.linear_reg_time_limit()
        self._jobs = simparser.SimParser.parse_pickle_job(log, regr.predict)
    ...
    def run_end(self, end_job_id=None):
        # step_sec = 1
        while True:
            #logging.info("Entry: Current simulation time: {}".format(self._time))
            if self._queue.empty():
                #print(f"it's upper branch: queue empty")
                while True:
                ...
                #print(f"it's lower branch: next_submit_time:{next_submit_time},next_job_id:{next_job_id}")
                ...
                while True:
                    ...
                    if submit_time == next_submit_time:
                        ...
                        break
                #print(f"the simulator is letting scheduler run {next_submit_time - self._time}")
                end, end_job_info = self._scheduler.run(next_submit_time - self._time, end_job_id)
                self._scheduler.submit(submit_job)
                if end:
                    #print(f"end job info 1 : {end_job_info[1]}")
                    #print(f"    end")
                    #self._time = end_job_info[1] + timedelta(seconds=int(self._mode))
                    # At the end, we sync the simulator time and scheduler time. 
                    # We don't want to change the scheduler time to previous, so we only push the simulator time to late
                    self._time = self._scheduler._time
                    scheduler_time = self._scheduler._time
                    #print(f"COMPARE: self time {self._time}, scheduler time {scheduler_time}")
                else:
                    #print(f"    not end")
    ...
    def run_time(self, time):
        steps = int(time.total_seconds() / int(self._mode))
        for i in range(steps):
        # for _ in range(0, int(time.total_seconds() / int(self._mode))):
            debug_time = self._time.strftime("%Y-%m-%dT%H:%M:%S")
            start_time = self._time
            # print(f"[Simulator] Step {i+1}: start time is: {start_time.strftime('%Y-%m-%dT%H:%M:%S')}")
            submit_jobs = []
            while True:
                if self._queue.empty():
                    break
            ...
            self._time += timedelta(seconds=int(self._mode))
            # print(f"[Simulator] Step {i+1}: after forward one step: {self._time.strftime('%Y-%m-%dT%H:%M:%S')}")
        return True
    ...
    def run_group_dependency(self, groups, infer_func, train_freq=300, infer_freq=1, sample_time_step=144,
        ...
        index_per_group = []
        infer_counter = 0
        infer_bound_running_job = None
        infer_bound_pending_job = None
        infer_bound_avail_nodes = self._scheduler.avail_nodes
        ...
        while True:
            ...
            for i in range(0, len(groups)):
                ...
                # TODO: Only two jobs for early age
                if early_age and self._time > next_job.submit:
                    next_job.early_priority += self._step_priority
                        ...
                        if infer_lower_bound <= expected_reward <= infer_upper_bound:
                            ...
                            logging.info(
                                "Simulator Time: {}, Successor job submission (Pred: {}, Succ: {})".format(self._time,
                                                                                                           pred_job.job_id,
                                                                                                           next_job.job_id))
                            # TODO: Only support one group
                            infer_bound_running_job = self.running_job_logs
                            infer_bound_pending_job = copy.deepcopy(self.pending_job_status)
                            infer_bound_avail_nodes = self._scheduler.avail_nodes
            ...
            if curr_reward == total_reward:
                if train_func is not None:
                    while len(self._active_reward) != 0:
                        ...
                        logging.info(
                            f"Simulator Time: {self._time}, Complete a train batch of {len(job_group_dataset)} samples.")
                return infer_bound_pending_job, infer_bound_running_job, infer_bound_avail_nodes
            # train_counter += step
            infer_counter += step
    ...
    def run_single_job(self, 
        ...
        while not finished:
            ...
            if sample_counter == 0:
                ...
                mini_batch_q = []
                mini_batch_m = []        
                # 收集 waiting/pending 列表
                for job_pend in sim_state.pending_jobs:
                    wait_sec = int((self._time - job_pend.submit).total_seconds())
                    mini_batch_q.append((job_pend.time_limit, 
                                         job_pend.nodes,
                                         wait_sec,
                                         0))  # 目前无runtime => 0        
                # 收集 running job
                for _, job_log in sim_state.running_jobs.items():
                    ...
                    mini_batch_m.append((job_log.job.time_limit,
                                         job_log.job.nodes,
                                         wait_sec,
                                         run_sec))        
                # 如果 self._global_history 已满, pop 最早的
                if len(self._global_history) == sample_time_step:
                    self._global_history.pop(0)        
                # 记录一下当前时刻 pending/running
                self._global_history.append([mini_batch_q, mini_batch_m])        
            sample_counter += step
            if sample_counter >= sample_time_length:
                sample_counter = 0        
            ...
            self.run_time(timedelta(seconds=step * int(self._mode)))        
            # c) 判断是否全部作业完成 => 
            if (len(self._scheduler.queue_state) == 0 
                and len(self._scheduler.running_jobs_state) == 0):
                finished = True        
        # 3) 返回收集到的环境采样信息
        return self._global_history

                    
    def linear_reg_time_limit(self):
        duration = []
        timelimit = []
        for key, value in self._jobs.items():
