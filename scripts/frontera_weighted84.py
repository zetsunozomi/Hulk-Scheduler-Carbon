"""Slurm entry point: restore minimal C84 fixed inputs, then the existing weighted PPO."""
import argparse
import os
...
def prepare_fixed(bundle, source, check=False):
    ...
    from weighted84_trial import source_inputs

    source = Path(source).resolve()
    require(bundle.raw['cluster']['nodes'] == 84 and bundle.raw['cluster']['allowed_nodes'] == [4, 16, 64],
            'Expected C84 and actions 4/16/64')
    marker = source/'frontera-fixed-plan.json'
    contract = {'kind': 'frontera_fixed84_v1', 'config_sha256': bundle.manifest['config_sha256'],
    ...
    if all((source/name).is_file() for name in required):
        ...
        print(f'Existing C84 fixed inputs verified: {source}', flush=True)
        return
    if source.exists():
    ...
    with probe_lock(source, source.exists(), '.frontera-fixed.lock'):
        ...
        source_inputs(bundle, source)
    print('Minimal C84 fixed preparation complete. No wait probes or planner fitting.', flush=True)


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--config', default='configs/amsp-frontera-7b-c84.development.json')
    parser.add_argument('--source', default='results/amsp-frontera-7b-c84-fixed-frontera')
    ...
    from weighted84_trial import run
