...
if [[ "${1:-}" == '--help' || "${1:-}" == '-h' ]]; then
  cat <<'USAGE'
Login node:      qsub scripts/sophia_actor_interaction.sh
Resume:          qsub -v CARBON_RESUME=1 scripts/sophia_actor_interaction.sh
PBS interactive: bash scripts/sophia_actor_interaction.sh [--resume]
One fresh seed11 PPO with explicit actor state/action products, 64 iterations.
Same environment, budgets and training objective; no planner rerun or test access.
Console output is also saved to out/actor-interaction.<job>.<UTC>.<unique>.log.
Overrides: CARBON_CONFIG, CARBON_PILOT, CARBON_MAIN, CARBON_DIAGNOSIS,
CARBON_OUTPUT, CARBON_PYTHON (use qsub -v for batch jobs).
USAGE
  exit 0
fi
if [[ -z "${PBS_JOBID:-}" ]]; then
...
cd "$repo_root"
mkdir -p out
log_path="$(mktemp "out/actor-interaction.${PBS_JOBID}.$(date -u +%Y%m%dT%H%M%SZ).XXXXXX.log")"
...
output_path="${CARBON_OUTPUT:-results/amsp-interaction-frontera-7b-seed11}"
printf 'PBS job: %s\nRepository: %s\nOutput: %s\n' "$PBS_JOBID" "$repo_root" "$output_path"
exec "$python_bin" -u -B scripts/actor_interaction_trial.py \
