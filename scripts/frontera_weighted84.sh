#!/bin/bash
#SBATCH --job-name=carbon-weighted84
...
if [[ "${1:-}" == '--help' || "${1:-}" == '-h' ]]; then
  cat <<'USAGE'
Login check (no replay): bash scripts/frontera_weighted84.sh --check
Login submission:       mkdir -p out; sbatch scripts/frontera_weighted84.sh
Batch resume:           sbatch scripts/frontera_weighted84.sh --resume
Interactive compute:    bash scripts/frontera_weighted84.sh [--resume]
Stages: --stage fixed | train | validate | all (default all).
Missing fixed source is rebuilt on the compute node; no wait probes or old PPO.
...
cd "$repo_root"
mkdir -p out
log_path="$(mktemp "out/frontera-weighted84.${SLURM_JOB_ID:-check}.$(date -u +%Y%m%dT%H%M%SZ).XXXXXX.log")"
...
exec "$python_bin" -u -B scripts/frontera_weighted84.py \
  --source "${CARBON_SOURCE:-results/amsp-frontera-7b-c84-fixed-frontera}" \
