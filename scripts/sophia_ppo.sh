#!/usr/bin/env bash
# Sophia local launcher; ignored by Git. Based on scripts/cluster_ppo.sh.
...
if [[ $# -lt 5 ]]; then
  echo 'Usage: bash scripts/sophia_ppo.sh CONFIG OUTPUT WAIT_MODEL REFERENCES ITERATIONS [train-ppo options...]' >&2
  exit 2
fi
