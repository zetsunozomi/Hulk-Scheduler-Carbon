#!/usr/bin/env bash
# Sophia local launcher; ignored by Git. Based on scripts/cluster_test.sh.
...
if [[ $# -lt 5 ]]; then
  echo 'Usage: bash scripts/sophia_test.sh CONFIG OUTPUT WAIT_MODEL SELECTION CHECKPOINT_DIR [CHECKPOINT_DIR ...]' >&2
  exit 2
fi
