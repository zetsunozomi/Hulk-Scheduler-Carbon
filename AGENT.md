...
5. 后续新实验默认只跑一个种子，固定 seed=11；只有用户明确要求时才增加种子。已运行的多种子配置和结果保留，不按结果挑选或删减种子。
6. 实验结果默认精简回传：运行配置、曲线/汇总表和单份 Slurm .out；完整逐任务记录、生成 trace 与重复日志留在集群。机会实验的 seed-* 目录整体忽略，不为画图要求传完整记录。
